...
for($i=0;$i<$count_results;$i++){
	...
	if ($visit_id != $results[$i]['visit_id'] || $results[$i]['visit_id'] == 0){
		...
		if (!empty($results[$i]['other_ip'])){
			...
			$other_ip_address = "<a class='text-filter' title='".htmlentities(sprintf(__("Filter results where ther user's real IP equals %s",'wp-slimstat-view'), ENT_QUOTES, 'UTF-8'), $results[$i]['other_ip'])."' href='".wp_slimstat_boxes::$current_screen_url.'&amp;fs='.wp_slimstat_boxes::replace_query_arg('other_ip', $results[$i]['other_ip'])."'>(".__('Originating IP','wp-slimstat-view').": {$results[$i]['other_ip']})</a>";
		}
	...
	if (!empty($results[$i]['resource'])){
		...
		if ($post_id > 0){
			$results[$i]['resource'] = "<a class='url' target='_blank' title='".htmlentities(__('Open this post in a new window','wp-slimstat-view'), ENT_QUOTES, 'UTF-8')."' href='{$results[$i]['resource']}'></a>".get_the_title($post_id);
		}
		else{
			$results[$i]['resource'] = "<a class='url' target='_blank' title='".htmlentities(__('Open this page in a new window','wp-slimstat-view'), ENT_QUOTES, 'UTF-8')."' href='{$results[$i]['resource']}'></a>".rawurldecode($results[$i]['resource']);
		}
	}
	if (empty($results[$i]['resource'])){
	...
	if (!empty($results[$i]['searchterms'])){
		...
		if (!empty($matches) && !empty($query_formats[$matches[1]])){
			$results[$i]['searchterms'] = "<a class='url' target='_blank' title='".htmlentities(__('Go to the corresponding search engine result page','wp-slimstat-view'), ENT_QUOTES, 'UTF-8')."' href='http://{$results[$i]['domain']}/".$query_formats[$matches[1]].'='.rawurlencode($results[$i]['searchterms'])."'></a> ".htmlentities($results[$i]['searchterms'], ENT_QUOTES, 'UTF-8');
		}
		else{
	...
	$results[$i]['domain'] = (!empty($results[$i]['domain']) && empty($results[$i]['searchterms']))?"<em class='domain'><a class='url' target='_blank' title='".htmlentities(__('Open in a new window','wp-slimstat-view'), ENT_QUOTES, 'UTF-8')."' href='{$results[$i]['referer']}'></a> {$results[$i]['domain']}</em>":'';
